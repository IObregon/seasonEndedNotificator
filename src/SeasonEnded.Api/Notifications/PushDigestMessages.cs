...
public static class PushDigestMessages
{
    ...
    private static string BuildEnglish(List<DigestCandidate> items)
    {
        ...
        var body = items.Count == 1
            ? $"{first.ShowTitle} Season {first.SeasonNumber} has ended!"
            : $"{items.Count} seasons have ended, including {first.ShowTitle} Season {first.SeasonNumber}!";
    ...
    private static string BuildSpanish(List<DigestCandidate> items)
    {
        var first = items[0];
        var body = items.Count == 1
            ? $"Temporada {first.SeasonNumber} de {first.ShowTitle} ha finalizado!"
            : $"{items.Count} temporadas han finalizado, incluyendo {first.ShowTitle} Temporada {first.SeasonNumber}!";
